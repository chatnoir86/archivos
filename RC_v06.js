...
//Creando los elementos y dándoles estilo.
...
var E={
hide:function(id){ $(id).style.visibility="hidden"; },
...
var rcBR={
W3CDOMSupported:document.getElementById&&document.createElement,
...
{ 
	...
	document.cookie=_13+"="+escape(_14)+_17+"; path=/"; 
},
...
{
this.rcFC.socket.playSound(id); 
}
reproducir('join')

// llamando al .swf
rcFC.socket.playSound('join')
...
//Esta funcion recibe los ''comandos'' que se envian al servidor
...
/*
Aqui x ejemplo pondriamos tres valores. Sindo los dos primeros un numeros, y desde el tercero cadenas de texto.
rcFC.receive()
**fail :/
*/
...
//++
var rcUtil={
trim:function(src){ src=src.replace(/[\u00a0\u00ad\s]+/g," "); return src.replace(/^[ \s]+|[ \s]+$/,""); },
html2bb:function(src)
{ 
return rcUtil.trim(rcUtil.rgb2hexC(rcUtil.re(src,new Array({m:/&nbsp;/gi,r:" "},
{m:/&amp;/gi,r:"&"},
{m:/class="Apple-style-span"/gi,r:""},
{m:/<br>|<br\/>/gi,r:""},
{m:/<b>(.+?)<\/b>/gi,r:"[b]$1[/b]"},
{m:/<i>(.+?)<\/i>/gi,r:"[i]$1[/i]"},
{m:/<strong>(.+?)<\/strong>/gi,r:"[b]$1[/b]"},
{m:/<em>(.+?)<\/em>/gi,r:"[i]$1[/i]"},
{m:/<u>(.+?)<\/u>/gi,r:"[u]$1[/u]"},
{m:/<span\s+(style="[^<>]*?)font-style:\s*italic\s*;([^<>]*?")>([^<]+?)<\/span>/gi,r:"<span $1 $2>[i]$3[/i]</span>"},
{m:/<span\s+(style="[^<>]*?)font-weight:\s*bold\s*;([^<>]*?")>([^<]+?)<\/span>/gi,r:"<span $1 $2>[b]$3[/b]</span>"},
{m:/<span\s+(style="[^<>]*?)text-decoration:\s*underline\s*;([^<>]*?")>([^<]+?)<\/span>/gi,r:"<span $1 $2>[u]$3[/u]</span>"},
{m:/<span\s+(style="[^<>]*?)color:\s*(.+?)\s*;([^<>]*?")>(.+?)<\/span>/gi,r:"<span $1 $3>[c=$2]$4[/c]</span>"},
{m:/<span\s+style="\s*">(.*?)<\/span>/gi,r:"$1"},
{m:/<font\s+color="?(.+?)"?>(.+?)<\/font>/gi,r:"[c=$1]$2[/c]"},
{m:/<(.|\n)+?>/gi,r:""}
)))); 
},
...
{ 
return rcUtil.re((src),new Array({m:/\[b\](.+?)\[\/b\]/gi,r:"<span style=\"font-weight: bold;\">$1</span>"},
{m:/\[i\](.+?)\[\/i\]/gi,r:"<span style=\"font-style: italic;\">$1</span>"},
{m:/\[u\](.+?)\[\/u\]/gi,r:"<span style=\"text-decoration: underline;\">$1</span>"},
{m:/\[c=(#[a-f\d]{6})\](.+?)\[\/c\]/gi,r:"<span style=\"color: $1;\">$2</span>"})); 
},

bb2plain:function(src)
{ 
return rcUtil.trim(rcUtil.re((src),new Array({m:/\[b\](.+?)\[\/b\]/gi,r:"$1"},
{m:/\[i\](.+?)\[\/i\]/gi,r:"$1"},
{m:/\[u\](.+?)\[\/u\]/gi,r:"$1"},
{m:/\[c=(#[a-f\d]{6})\](.+?)\[\/c\]/gi,r:"$2"}))); 
},
...
{
var res="";
var b=src.indexOf("http://"); 
if(b!=-1)
	{ 
	var e=src.indexOf(" ",b); 
	var _3b=src.substring(b,e!=-1?e:src.length); 
	res=src.substring(0,b)+"<a target=\"_blank\" href=\""+_3b.replace(/&shy;|<wbr\/>/g,"")+"\">"+_3b+"</a> "; 
		if(e!=-1)
		{ 
		res+=src.substring(e); 
		} 
...
//
var CP={
bind:function(){ var cnt="<table cellspacing=\"2\" border=\"0\" cellpadding=\"0\"><tr>"; for(var i=27;i<47;i++){ if(i>27&&(i+3)%10==0){ cnt+="</tr><tr>"; } cnt+="<td><a style=\"display: block; width: 18px; height: 22px; font-size:1px; background: "+T.c[i]+";\" onclick=\"return IN.sCol("+i+");\">&nbsp;</a></td>"; } cnt+="</tr></table>"; $("CP").innerHTML=cnt; }}; 
...
//+++
var BL={
appName:"RCH: "+navigator.userAgent+" "+navigator.platform,
loggedIn:null,
...
{ 
	if(_aa=="100")								// PRIMER _aa:
		...
		CA.addServerMessage(CA.cb,"<b>"+'Antiflooding by +Positivo+ :D'+"</b>"); }//advertencia del flooding
	else{ 
		if(cID=="104"){
		CA.addServerMessage(CA.cb,"<b>"+rcLn["WID"]+"</b>"); }//Warning! You may be kicked for inactivity."
	else{ 
		if(cID=="107"){ 
		CA.addServerMessage(CA.cb,"<b>"+rcLn["WPD"]+"</b>"); }//"Warning! You may be kicked for profanity."
	else{ 
		if(cID=="106"){ 
		L.showLogin(); 
		LDL.warn(rcLn["LDE9"]); //"Room closed." o sea cierra el chat.
		BL.dE=true; }
	else{ 
		if(cID=="103"){ 
		BL.dE=true; 
			...
			L.showLogin(); 
			LDL.warn(rcLn["LDE11"]); }//"You have been kicked for flooding."
		else{ 
			if(cID=="108"){ 
			BL.dE=true; if(_ac[0]!=""){ 
			window.location=_ac[0]; } 
			L.showLogin(); 
			LDL.warn(rcLn["LDE12"]); }//"You have been kicked for profanity."
		else{ 
			if(cID=="109"){ 
			BL.dE=true; 
			if(_ac[0]!=""){ 
			window.location=_ac[0]; } 
			L.showLogin(); 
			LDL.warn(rcLn["LDE13"]); }//"You have been kicked by chat operator."
		else{ 
			if(cID=="207"){ 
			BL.dE=true; 
			if(_ac[0]!=""){ 
			window.location=_ac[0]; } 
			L.showLogin(); 
			LDL.warn(rcLn["LDE14"]); }//"You have logged on from another location."
			} 
			} 
			} 
		} 
	...
	CA.addServerMessage(CA.cb,_ac[3]+" "+_ac[4]); 
		...
		{ 
		L.showLogin(); 
		LDL.warn(rcLn["LDE3"]); //"This nickname is in use."
		BL.dE=true; 
		}
	else{ 
		if(cID=="212")
		{ 
		L.showLogin(); 
		LDL.warn(rcLn["LDE4"]+"<br/>"+_ac[0]); //"Server closed."
		BL.dE=true; 
		}
	else{ 
		if(cID=="211")
		{ 
		L.showLogin(); 
		LDL.warn(rcLn["LDE5"]); //"Server full."
		BL.dE=true; 
		}
	else{ 
		if(cID=="213")
		{ 
		L.showLogin(); 
		LDL.warn(rcLn["LDE7"]); //"Invalid server."
		BL.dE=true; 
		}
	else{ 
		if(cID=="208")
		{ 
		L.showLogin(); 
		LDL.warn(rcLn["LDE6"]); //"Invalid input."
		BL.dE=true; 
		}
	else{ 
		if(cID=="205")
		{ 
		L.showLogin(); 
		LDL.warn(rcLn["LDE8"]); //"Authentication failed."
		BL.dE=true; 
		}
	else{ 
		if(cID=="210")
		{ 
		L.showLogin(); 
		LDL.warn(rcLn["LDE10"]); //"This nickname or IP address is banned."
		BL.dE=true; 
		} 
		} 
		} 
	...
	{ 
		...
		var u=ULS.getUser(_ac[0]); 
			...
			{ 
				if(_ac[1]!="")
				{ 
				CA.addServerMessage(CA.mb,rcLn["CUR"],u.nick,_ac[1]); //deja la sala se va a...
				}
				else{ 
				CA.addServerMessage(CA.mb,rcLn["CUL"],u.nick); //"{0} left the room. (Logged out)"
				} 
			} 
	...
	{ 
		...
		if(cID=="505")
		{ 
		CA.addServerMessage(CA.cb,rcLn["CNU"]); //"User is not connected."
		}
		else{ 
		if(cID=="602"||cID=="601")
		{ 
	...
	{ 
		...
		{ 
			if(_ac[0]==BL.uID)
			{ 
			CA.addServerMessage(CA.cb,"<b>"+rcLn["AYWB"]+"</b>",ULS.getUser(_ac[1]).nick); //"You have been warned by {0}."
			}
			else
			{ 
			CA.addServerMessage(CA.cb,rcLn["AWB"],ULS.getUser(_ac[0]).nick,ULS.getUser(_ac[1]).nick); //"{0} has been warned by {1}." *ha sido...
			}
		}
		else{ 
		if(cID=="715")
...
{ 
	if(msg.length>BL.mL)
		{ 
		msg=msg.substring(0,BL.mL); 
		}
var pl=rcUtil.bb2plain(msg); 
	if(pl.length<1)
		{ 
		return; 
		} 
var _b3=true; 
	if(pl.charAt(0)=="/"&&pl.length>1&&pl.charAt(1)!="/")
		{
		_b3=false; 
		var cmd=pl.substring(1); 
		var arg=""; 
		var si=cmd.indexOf(" "); 
		if(si!=-1)
			{ 
			arg=cmd.substring(si+1); 
			cmd=cmd.substring(0,si); 
			} 
	if(cmd=="pass")
		{ 
		rcFC.send("700","701",arg); 
		}
	else
		{ 
		if(cmd=="user")
		...
		{ 
			if(cmd=="highlight")
			{ 
			var id=ULS.getID(arg); 
				if(id!=null)
				{ 
				rcFC.send("700","761",id); 
				} 
			} 
		} 
		} 
	...
	else
	{
		...
		var ci=arg.indexOf(":"); 
			if(ci!=-1)
			{ 
			_b8=arg.substring(0,ci); 
			if(_b8==BL.nick)
			{ 
			return; 
			} 
			var id=ULS.getID(_b8); 
			if(id!=null)
			{ 
			var _ba=msg.replace("/msg ",""); 
			_ba=rcUtil.trim(_ba.replace(_b8+":","")); 
				if(rcUtil.bb2plain(_ba).length<1)
				{ 
				return; 
				} 
				rcFC.send("500","503",id,_ba); 
				CA.addUserMessage(4,id,_ba.replace("& ","&amp; "));
			} 
			} 
		} 
	} 
...
{ 
	if(to!=-1)
	{ 
	rcFC.send("500","503",to,msg); 
	}
	else
	{ 
	rcFC.send("500","502","",msg); 
	} 
	CA.addUserMessage(4,BL.uID,msg.replace("& ","&amp; ")); 
} 
},

setAway:function(_bb){ rcFC.send("500",(_bb)?"602":"601"); }}; 
...
bind:function()
			...
			LDL.lDD=$("LDD"); 
			LDL.lDM=$("LDM"); 
			LDL.lDLB.onclick=LDL.doLogin; 
			LDL.lO=$("LO"); 
			$("LDO").onclick=LDL.shOpts; 
			LDL.lDNN.setAttribute("autocomplete","off"); 
			if(rcBR.ie)
			{ 
			LDL.lDNN.attachEvent("onkeydown",LDL.kPress); 
			}
			else
			{ 
			LDL.lDNN.addEventListener("keypress",LDL.kPress,false); 
			} 
			E.hide(LDL.lDD); 
			},
...
aCL:function(err){ if(LDL.aCP){ return; } LDL.aCP=true; if(err){ BL.au=""; }else{ var i=$("aii"); if(i.width>200||i.height>200){ BL.au=""; } } rcFC.connect(BL.host,BL.port); }}; 
...
ap:function(id)
{ 
for(var i=0;i<MP.mb.length;i++)
	{ 
		if(MP.mb[i]!=null&&MP.mb[i].id==id)
			{ 
				rcFC.send("500","612",MP.mb[i].sid,rcUtil.unescapeHTML(MP.mb[i].body)); 
				MP.mb.splice(i,1); break; 
			} 
	} 
MP.redraw(); 
},
...
{ 
	for(var i=0;i<MP.mb.length;i++)
		...
		if(MP.mb[i]!=null&&MP.mb[i].id==id)
			{ 
				rcFC.send("500","613",MP.mb[i].sid,rcUtil.unescapeHTML(MP.mb[i].body)); 
				MP.mb.splice(i,1); break; 
			} 
		} 
MP.redraw(); 
...
bind:function(){ MP.MD=$("MD"); MP.ML=$("ML"); MP.MS=$("MS"); }}; 
...
updateCounts:function(r)
{ 
	var rr=r.split(rcMTP.GS); 
	for(var i=0;i<rr.length;i++)
		{ 
		if(rr[i].length>0)
			{ 
			var p=rr[i].split(rcMTP.RS); 
				for(var j=0;j<RLS.rooms.length;j++)
					{ 
					if(RLS.rooms[j].id==p[0])
						{ 
						RLS.rooms[j].users=p[1]; 
						} 
					} 
			} 
		} 
...
SD.s=""; 
	for(var i=0;i<11;i++)
	{ 
	SD.s+=($("s"+i).checked)?1:0; 
	} 
rcCookie.set("s",SD.s,365); 
ULS.fs=(SD.s.charAt(7)=="1")?15:13; 
ULS.sA=(SD.s.charAt(3)=="1"); 
ULS.sM=(SD.s.charAt(4)=="1")?1:0; 
ULS.sort(); 
ULS.redraw(); 
RLS.fs=(SD.s.charAt(7)=="1")?15:13; 
RLS.sort(); 
RLS.redraw(); 
CA.sA=(SD.s.charAt(0)=="1"); 
CA.sE=(SD.s.charAt(1)=="1"); 
CA.sF=(SD.s.charAt(2)=="1"); 
CA.sJ=(SD.s.charAt(5)=="1"); 
CA.iP=(SD.s.charAt(6)=="1"); 
CA.psJ=(SD.s.charAt(8)=="1"); 
CA.psM=(SD.s.charAt(9)=="1"); 
CA.psP=(SD.s.charAt(10)=="1"); 
CA.fs=(SD.s.charAt(7)=="1")?15:13; 
CA.redraw(); 
IN.fs=(SD.s.charAt(7)=="1")?15:13; 
IN.CB.style.fontSize=IN.fs+"px"; 
IN.doFocus(); 
}
...
{ 
TB.tabs=new Array(); 
TB.addTab(-1,rcLn["GCL"]); 
TB.selTab(-1); 
TB.mT=TB.tabs[0]; 
TB.OT.style.borderBottom="1px solid "+T.c[4]; 
TB.TABS.style.display="none"; 
},
...
setUsers:function(u)
		{ 
		ULS.users.splice(0,ULS.users.length); 
		var uu=u.split(rcMTP.GS); 
...
addUser:function(uu)
		{ 
		var p=uu.split(rcMTP.RS); 
		if(p[0]==BL.uID){ p[2]="-"; } 
		TB.fonTab(p[1],p[0]); 
		var bl=ULS.isBlocked(p[1]); 
		ULS.users.splice(0,0,{id:p[0],nick:p[1],status:p[2],role:p[3],bl:bl,avatar:p[4],profile:p[5]}); 
		return p[1]; 
		},
